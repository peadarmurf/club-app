...
/**
 * Event is fired when reset button is pressed to inform that facet filter lists need to be reset. 
 *
		...
		oPopover = new sap.m.Popover({ 
			...
			afterClose: function(oEvent) {
				...
				// is the popover causing facet filter item checkbox selection to not display the check mark when the item is selected.
					that.destroyAggregation("popover");
			},
...
sap.m.FacetFilter.prototype._setButtonText = function(oList) {
	...
	if(oButton) { // Button may not be created yet if FFL.setTitle() is called before the button is rendered the first time
		...
		var iLength = Math.max(aSelectedItems.length, aSelectedKeyNames.length);

		if (iLength > 0) {

			if (iLength === 1) { // Use selected item value for button label if only one selected
				var sSelectedItemText = aSelectedItems.length === iLength ? 
						aSelectedItems[0].getText() : 
						oList._oSelectedKeys[aSelectedKeyNames[0]];
...
sap.m.FacetFilter.prototype._getFacetDialogNavContainer = function() {
	...
	var oFilterItemsPage = this._createFilterItemsPage();
	oNavContainer.addPage(oFacetPage);
	oNavContainer.addPage(oFilterItemsPage);
	...
	oNavContainer.attachAfterNavigate(function(oEvent) {
		...
		if (oToPage === oFacetPage) {
			...
			oFilterItemsPage.destroySubHeader();
			...
			oFilterItemsPage.destroyContent(); // Destroy the select all checkbox bar	
...
sap.m.FacetFilter.prototype._closeDialog = function() {
	...
	if(oDialog && oDialog.isOpen()) {
		...
		// Make sure we fire list close for the currently displayed list
		var oNavContainer = oDialog.getContent()[0];
		var oCurrentPage = oNavContainer.getCurrentPage();
		var oFacetItemsPage = oNavContainer.getPages()[1];
		if(oCurrentPage === oFacetItemsPage) {
			this._displayedList._fireListCloseEvent();
		}	
		oDialog.close();	
	}
};
...
sap.m.FacetFilter.prototype._navToFilterItemsPage = function(oFacetListItem) {
	...
	var oFacetItemsPage = oNavCont.getPages()[1];
	
	// This page instance is used to display content for every facet filter list, so remove any prior content, if any. 
	oFacetItemsPage.destroyAggregation("content", true);
	
	// Add the facet filter list
	this._moveListToDisplayContainer(oFacetFilterList, oFacetItemsPage);
	
	// Add the search field bar. The bar is destroyed from NavContainer.afterNavigate.
	oFacetItemsPage.setSubHeader(this._createFilterItemsSearchFieldBar(oFacetFilterList));
	...
	if(oCheckboxBar) {
		oFacetItemsPage.insertContent(oCheckboxBar, 0);	
	}

	oFacetItemsPage.setTitle(oFacetFilterList.getTitle());
	
	oFacetFilterList.fireListOpen({});
	oNavCont.to(oFacetItemsPage);
};
...
sap.m.FacetFilter.prototype._moveListToDisplayContainer = function(oList, oContainer) {
	...
	oList.setAssociation("facetFilter", this, true); //Make the FacetFilter available from the list even after it is moved
	...
	if(!oSummaryBar) {
		...
		oSummaryBar = new sap.m.Toolbar({
			...
			press : function(oEvent) {
				...
				if(!that.resetPressed) { 
					that.openFilterDialog();					
				}
				that.resetPressed = false;
			}
		});	
...
sap.m.FacetFilter.prototype._createResetButton = function() { 
	...
	var oButton = new sap.m.Button({
		...
		press : function(oEvent) {
			...
			// resetPressed is a work around to prevent the dialog from opening if the
			// reset button is pressed. This has been fixed in the Toolbar in 1.20+.
			that.resetPressed = true;
			that.fireReset();			
